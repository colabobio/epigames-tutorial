export const translations = {
  en: {
    prev: "Previous",
    next: "Next",
    index: "Tutorial Index",
    steps: [
      {
        title: "Your Avatar",
        lead: "The avatar is your digital proxy during the epigame. Your avatar's health depends on your choices."
      },
      {
        title: "Quarantine or Stay Active?",
        lead: "Every day, you will have to choose to quarantine your avatar or not. Quarantine keeps you safe from infection. You get baseline points either way, but you can earn extra points by not quarantining."
      },
      {
        title: "Social Contacts & Bonuses",
        lead: "Each contact longer than 5 minutes with another player will get you a point bonus (and you need to keep Bluetooth on). However, you risk getting your avatar sick if you encounter infected players."
      },
      {
        title: "The Shop",
        lead: "Use your points in the shop to buy masks, rapid tests, antiviral medicine, and even apparel for your avatar! Use your points wisely, as your final point score may help you win lottery prizes at the end of the epigame."
      },
      {
        title: "Getting Sick",
        lead: "If your avatar gets sick, you will make very few daily points until recovery. And if your avatar dies, you will lose all your points!"
      },
      {
        title: "Extra Challenge!",
        lead: "Are you up for a challenge? This part of the epigame is optional, scroll down to learn more!",
        page6Steps: [
          { text: "Got Sick? Grab the free sequencing tool from the shop." },
          { text: "Your kit gives you a DNA fragment, just one piece of the full virus. Share it with friends however you like: message, email, or cloud." },
          { text: "Everyone gets a different piece of the DNA. Team up to complete the sequence! How you do it is up to you." },
          { text: "Think you've cracked the full sequence? Submit your guess at sequencing.epidemica.info and unlock a special reward!" },
          { sectionTitle: "How to assemble the virus genome from the fragments?", sectionBody: "Let's say the sequence is \"ATGCGTACGA\"" },
          { text: "Each player holds part of the sequence. By finding overlaps, they can piece together the original DNA sequence." },
          { text: "There you have it! That's how you solve a sequence using overlapping fragments. But this was just a simple example. Your real challenge will be assembling a sequence that's hundreds of bases long! Get ready for a more complex puzzle!" }
        ]
      }
    ]
  },
  es: {
    prev: "Anterior",
    next: "Siguiente",
    index: "Índice del Tutorial",
    steps: [
      {
        title: "Tu Avatar",
        lead: "El avatar es tu representante digital durante el epigame. La salud de tu avatar depende de tus decisiones."
      },
      {
        title: "¿Cuarentena o Mantenerse Activo?",
        lead: "Cada día, tendrás que elegir si poner en cuarentena a tu avatar o no. La cuarentena te protege de la infección. De cualquier manera obtienes puntos base, pero puedes ganar puntos extra si no haces cuarentena."
      },
      {
        title: "Contactos Sociales y Bonificaciones",
        lead: "Cada contacto de más de 5 minutos con otro jugador te dará una bonificación de puntos (y necesitas mantener el Bluetooth activado). Sin embargo, arriesgas que tu avatar se enferme si encuentras jugadores infectados."
      },
      {
        title: "La Tienda",
        lead: "¡Usa tus puntos en la tienda para comprar mascarillas, pruebas rápidas, medicamentos antivirales e incluso ropa para tu avatar! Usa tus puntos con cuidado, ya que tu puntuación final puede ayudarte a ganar premios de lotería al final del epigame."
      },
      {
        title: "Enfermarse",
        lead: "Si tu avatar se enferma, ganarás muy pocos puntos diarios hasta que se recupere. ¡Y si tu avatar muere, perderás todos tus puntos!"
      },
      {
        title: "¡Desafío Extra!",
        lead: "¿Estás listo para un desafío? Esta parte del epigame es opcional, ¡desplázate hacia abajo para saber más!",
        page6Steps: [
          { text: "¿Te enfermaste? Consigue la herramienta de secuenciación gratuita en la tienda." },
          { text: "Tu kit te da un fragmento de ADN, solo una pieza del virus completo. Compártelo con amigos como quieras: mensaje, correo o nube." },
          { text: "Cada jugador recibe una pieza diferente del ADN. ¡Únete a otros para completar la secuencia! Cómo lo hagan depende de ustedes." },
          { text: "¿Crees que descifrastes la secuencia completa? ¡Envía tu respuesta en sequencing.epidemica.info y desbloquea una recompensa especial!" },
          { sectionTitle: "¿Cómo ensamblar el genoma del virus a partir de los fragmentos?", sectionBody: "Digamos que la secuencia es \"ATGCGTACGA\"" },
          { text: "Cada jugador tiene parte de la secuencia. Al encontrar superposiciones, pueden reconstruir la secuencia de ADN original." },
          { text: "¡Ahí lo tienen! Así se resuelve una secuencia usando fragmentos superpuestos. Pero este fue solo un ejemplo simple. ¡Tu verdadero desafío será ensamblar una secuencia de cientos de bases! ¡Prepárate para un rompecabezas más complejo!" }
        ]
      }
    ]
  },
  ar: {
    prev: "السابق",
    next: "التالي",
    index: "فهرس التعليمات",
    steps: [
      {
        title: "شخصيتك الافتراضية",
        lead: "الشخصية الافتراضية (avatar) هي وكيلك الرقمي خلال اللعبة. تعتمد صحة شخصيتك على قراراتك."
      },
      {
        title: "الحجر الصحي أم البقاء نشطًا؟",
        lead: "كل يوم، يجب عليك اختيار ما إذا كنت ستضع شخصيتك في الحجر الصحي أم لا. يحميك الحجر الصحي من الإصابة. ستحصل على نقاط أساسية في كلتا الحالتين، لكن يمكنك كسب نقاط إضافية إذا لم تنفذ الحجر الصحي."
      },
      {
        title: "الاتصالات الاجتماعية والمكافآت",
        lead: "كل اتصال أطول من 5 دقائق مع لاعب آخر سيمنحك مكافأة نقاط (ويجب أن تبقي البلوتوث مفعّلاً). ومع ذلك، تخاطر بمرض شخصيتك إذا واجهت لاعبين مصابين."
      },
      {
        title: "المتجر",
        lead: "استخدم نقاطك في المتجر لشراء أقنعة، واختبارات سريعة، وأدوية مضادة للفيروسات، وحتى ملابس لشخصيتك! استخدم نقاطك بحكمة، فقد يساعدك مجموع نقاطك النهائي على الفوز بجوائز اليانصيب في نهاية اللعبة."
      },
      {
        title: "الإصابة بالمرض",
        lead: "إذا مرضت شخصيتك، ستكسب نقاطًا يومية قليلة جدًا حتى تتعافى. وإذا ماتت شخصيتك، ستفقد جميع نقاطك!"
      },
      {
        title: "تحدٍ إضافي!",
        lead: "هل أنت مستعد للتحدي؟ هذا الجزء من اللعبة اختياري، مرر لأسفل لمعرفة المزيد!",
        page6Steps: [
          { text: "هل مرضت؟ احصل على أداة التسلسل المجانية من المتجر." },
          { text: "تمنحك مجموعتك شريحة DNA، مجرد قطعة واحدة من الفيروس الكامل. شاركها مع أصدقائك بأي طريقة: رسالة، أو بريد إلكتروني، أو سحابة." },
          { text: "يحصل كل لاعب على قطعة مختلفة من الـ DNA. تعاون مع الآخرين لإكمال التسلسل! كيفية القيام بذلك متروك لكم." },
          { text: "هل تعتقد أنك حللت التسلسل الكامل؟ أرسل تخمينك على sequencing.epidemica.info وافتح مكافأة خاصة!" },
          { sectionTitle: "كيف تجمع جينوم الفيروس من الشظايا؟", sectionBody: "لنفترض أن التسلسل هو \"ATGCGTACGA\"" },
          { text: "يحمل كل لاعب جزءًا من التسلسل. من خلال إيجاد التداخلات، يمكنهم تجميع تسلسل DNA الأصلي." },
          { text: "ها هو ذا! هكذا تحل التسلسل باستخدام الشظايا المتداخلة. لكن هذا كان مجرد مثال بسيط. تحديك الحقيقي سيكون تجميع تسلسل يمتد لمئات من القواعد! استعد لأحجية أكثر تعقيدًا!" }
        ]
      }
    ]
  },
  vi: {
    prev: "Trước",
    next: "Tiếp theo",
    index: "Mục lục hướng dẫn",
    steps: [
      {
        title: "Nhân Vật Của Bạn",
        lead: "Nhân vật đại diện (avatar) là đại diện kỹ thuật số của bạn trong epigame. Sức khỏe của nhân vật phụ thuộc vào các lựa chọn của bạn."
      },
      {
        title: "Cách Ly hay Duy Trì Hoạt Động?",
        lead: "Mỗi ngày, bạn sẽ phải chọn có cách ly nhân vật của mình hay không. Cách ly giúp bạn an toàn khỏi lây nhiễm. Bạn nhận được điểm cơ bản dù thế nào, nhưng bạn có thể kiếm thêm điểm nếu không cách ly."
      },
      {
        title: "Tiếp Xúc Xã Hội & Phần Thưởng",
        lead: "Mỗi tiếp xúc dài hơn 5 phút với người chơi khác sẽ mang lại cho bạn một phần thưởng điểm (và bạn cần giữ Bluetooth bật). Tuy nhiên, bạn có nguy cơ làm nhân vật bị bệnh nếu gặp người chơi bị nhiễm."
      },
      {
        title: "Cửa Hàng",
        lead: "Dùng điểm của bạn trong cửa hàng để mua khẩu trang, xét nghiệm nhanh, thuốc kháng vi-rút, và thậm chí trang phục cho nhân vật! Hãy dùng điểm thật khôn ngoan, vì tổng điểm cuối cùng có thể giúp bạn giành giải xổ số khi kết thúc epigame."
      },
      {
        title: "Bị Bệnh",
        lead: "Nếu nhân vật bị bệnh, bạn sẽ kiếm được rất ít điểm mỗi ngày cho đến khi hồi phục. Và nếu nhân vật chết, bạn sẽ mất toàn bộ điểm!"
      },
      {
        title: "Thử Thách Thêm!",
        lead: "Bạn có sẵn sàng thử thách không? Phần này của epigame là tùy chọn, cuộn xuống để tìm hiểu thêm!",
        page6Steps: [
          { text: "Bị bệnh? Lấy công cụ giải trình tự miễn phí từ cửa hàng." },
          { text: "Bộ kit của bạn cho một đoạn DNA, chỉ một mảnh của toàn bộ vi-rút. Chia sẻ với bạn bè theo cách bạn thích: tin nhắn, email, hoặc đám mây." },
          { text: "Mỗi người chơi nhận một mảnh DNA khác nhau. Hợp tác để hoàn thành chuỗi! Cách thực hiện là tùy bạn." },
          { text: "Bạn nghĩ mình đã giải được chuỗi đầy đủ? Gửi đáp án tại sequencing.epidemica.info và mở khóa phần thưởng đặc biệt!" },
          { sectionTitle: "Làm thế nào để lắp ráp bộ gen virus từ các mảnh?", sectionBody: "Giả sử chuỗi là \"ATGCGTACGA\"" },
          { text: "Mỗi người chơi giữ một phần của chuỗi. Bằng cách tìm các đoạn chồng lấp, họ có thể ghép lại chuỗi DNA gốc." },
          { text: "Đó là cách giải chuỗi bằng các mảnh chồng lấp. Nhưng đây chỉ là ví dụ đơn giản. Thử thách thực sự của bạn sẽ là lắp ráp một chuỗi dài hàng trăm base! Hãy sẵn sàng cho một câu đố phức tạp hơn!" }
        ]
      }
    ]
  }
};
